class DvTasks {
	...
	getTodayTasks(args) {
		...
		return this.getTasksTable({
			...
			filterFn: (t) =>
				t.status != "Completed" &&
				t.dueDate &&
				t.dueDate?.hasSame(luxon.DateTime.fromISO(finalDate), "day"),
		});
	}
	...
	getOverdueTasks(args) {
		...
		return this.getTasksTable({
			...
			filterFn: (t) =>
				t.dueDate &&
				t.dueDate < luxon.DateTime.fromISO(finalDate) &&
				t.status != "Completed",
		});
	}
			...
			pages.map((t) => {
				...
				return [
					...
					createButton({
						...
						clickOverride: {
							click: update,
							params: ["Status", updateStr, t.file.path],
						},
					}),
				];
			})
